...
$encoded = kubectl --context docker-desktop -n argocd get secret argocd-initial-admin-secret `
  -o jsonpath='{.data.password}'
$decoded = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($encoded))
Write-Host $decoded
Write-Host ""

Write-Host "5) UI 접속: 아래 명령 실행 후 https://localhost:8443 접속 (인증서 경고는 무시)"
